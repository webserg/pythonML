...
def quick_sort(unsorted_list, start, end):
    if end < start:
        return
    pdx = start
    pdx = partitioning(unsorted_list, pdx, end)
    quick_sort(unsorted_list, start, pdx - 1)
    quick_sort(unsorted_list,  pdx + 1, end)


def partitioning(unsorted_list, pdx, end):
    j, i = pdx+1, pdx+1
    while j <= end:
        if unsorted_list[j] < unsorted_list[pdx]:
            unsorted_list[j], unsorted_list[i] = unsorted_list[i], unsorted_list[j]
            i += 1
        j += 1
    unsorted_list[pdx], unsorted_list[i - 1] = unsorted_list[i - 1], unsorted_list[pdx]
    return i - 1
